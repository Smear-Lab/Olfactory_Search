...
measurement = 'mean_performance' #best_performance,mean_performance,trial_duration,tortuosity,sniff_freq,sniff_number
concentration_list = ['p','v']

#ANALYSIS PREFERENCES
#General arena coordinates
nosepoke = [1127,354]
error_report = False #print errors when excluding data 
...
for mouse_id in subject_list:
    ...
    for concentration_setting in concentration_list:
            ...
            if session_type == concentration_setting:
                ...
                for current_trial in range (0,len(trial_summ)):
                    ...
                    if measurement == 'tortuosity':
                        distance_travelled = 0; frame = 0
                        if start * 80 > len(ts) or end * 80 > len(ts):
                            if error_report == True:
                                print mouse_id, session, current_trial, 'timestamp & trial timestamps dont match up'
                            continue
                        #Find first & last frame of trial in tracking data using timestamp file
                        startline = (np.abs(ts - start)).argmin()
                        endline = (np.abs(ts - end)).argmin()
        
                        find_crossing = 0
                        for frames in range(startline,endline+1):
                            if nx[frames] <= 421:
                                if find_crossing == 0:
                                    crossing_point = [nx[frames],ny[frames]]
                                    find_crossing = 1
                                if find_crossing == 1:
                                    continue
                            distance = np.sqrt(math.pow((nx[frames+1]-nx[frames]),2) + math.pow((ny[frames+1] - ny[frames]),2))
                            distance_travelled = distance_travelled + distance
                        shortest_distance = np.sqrt(math.pow(abs(crossing_point[0] - nosepoke[0]),2) + math.pow(abs(crossing_point[1] - nosepoke[1]),2))
                        if concsetting[current_trial] == 1:
                            measurement_values.append(distance_travelled/shortest_distance)
                            if session_type == 'p': p_list.append(distance_travelled/shortest_distance)
                            if session_type == 'v': v_list.append(distance_travelled/shortest_distance)
        ...
        if measurement == 'trial_duration' or measurement == 'tortuosity' or measurement == 'sniff_freq' or measurement == 'mean_performance':
...
if measurement == 'mean_performance':
    results = spystats.ranksums(mouse_list[:,0], mouse_list[:,1])
else: results = spystats.ranksums(p_list, v_list)
print results
